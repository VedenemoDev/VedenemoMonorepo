...
MODEL_AZ_NAME="FamilyUnitTreeCompositeWithDates"
MODEL_INSTANCE_ROOT_NAME="British Royal Family Unit Tree Composite With Dates"
VDOS_FILE="${REPO_ROOT}/.vedenemo/FamilyUnitTreeCompositeWithDates.vdos"
...
def require_expected_model_shape(api_description):
    ...
    for entity_az_name, attributes_by_name in expected_attributes.items():
        entity = entities.get(entity_az_name)
        if not entity:
            raise SystemExit(f"Loaded model is missing entity {entity_az_name}")
        attributes = {attribute["azName"]: attribute for attribute in entity.get("attributes", [])}
        for attribute_az_name, data_type in attributes_by_name.items():
            attribute = attributes.get(attribute_az_name)
            if not attribute:
                raise SystemExit(f"Loaded model is missing attribute {entity_az_name}.{attribute_az_name}")
            if attribute.get("dataType") != data_type:
                raise SystemExit(f"Loaded model attribute {entity_az_name}.{attribute_az_name} has unexpected dataType {attribute.get('dataType')}")
...
def compact_blank_values(values):
...
for person in PERSONS:
    values = compact_blank_values({key: person[key] for key in ["GivenNames", "FamilyName", "BirthDate", "DeathDate", "DisplayName", "Notes"]})
...
for family_unit in FAMILY_UNITS:
    values = compact_blank_values({key: family_unit[key] for key in ["Label", "StartDate", "EndDate", "RelationType", "Status", "Notes"]})
